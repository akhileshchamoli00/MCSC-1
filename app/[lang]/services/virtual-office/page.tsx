...
export async function generateMetadata(props: PageProps): Promise<Metadata> {
  ...
  return {
    ...
    alternates: {
      canonical: `https://www.mcsc.co.id/${lang}/services/virtual-office`,
      languages: {
...
export default async function VirtualOfficePage(props: PageProps) {
  const params = await props.params;
      ...
      <div className="container mx-auto px-4 pt-8 pb-0 max-w-6xl text-left">
        ...
        <section id="what-is-vo" className="scroll-mt-24 mb-16">
          <h2 className="text-3xl font-bold mb-6 text-foreground border-b border-border/50 dark:border-white/10 pb-3">
            {getTranslation(contentData.whatIsTitle)}
          </h2>
          <p className="text-base text-muted-foreground leading-relaxed mb-6">
            {getTranslation(contentData.whatIsDesc)}
          </p>
        </section>
        ...
        <section id="comparing-vo" className="scroll-mt-24 mb-16">
          <h2 className="text-3xl font-bold mb-6 text-foreground border-b border-border/50 dark:border-white/10 pb-3">
            {getTranslation(contentData.comparingTitle)}
          </h2>
        ...
        <section id="whats-included" className="scroll-mt-24 mb-16">
          ...
          <div className="grid gap-6 md:grid-cols-2 max-w-4xl">
            {/* North Jakarta */}
            <div className="p-6 rounded-2xl border border-border/50 dark:border-white/15 bg-background/40 hover:border-primary/40 hover:bg-primary/5 transition-all duration-300 backdrop-blur-sm shadow-sm flex flex-col group">
              ...
              <ul className="space-y-3 flex-grow">
                {contentData.whatsIncludedNorthBullets.map((bullet, idx) => (
            ...
            <div className="p-6 rounded-2xl border border-border/50 dark:border-white/15 bg-background/40 hover:border-primary/40 hover:bg-primary/5 transition-all duration-300 backdrop-blur-sm shadow-sm flex flex-col group">
              ...
              <ul className="space-y-3 flex-grow">
                {contentData.whatsIncludedTangerangBullets.map((bullet, idx) => (
                  <li key={idx} className="flex items-start gap-2.5 text-xs text-muted-foreground">
                    <CheckCircle2 className="h-4 w-4 text-primary shrink-0 mt-0.5" />
                    <span>{getTranslation(bullet)}</span>
                  </li>
        ...
        <section id="requirements-details" className="scroll-mt-24 mb-16">
          ...
          <div className="grid gap-6 md:grid-cols-2 lg:grid-cols-3 mb-8">
            ...
            <div className="p-6 rounded-2xl border border-border/50 dark:border-white/15 bg-background/45 backdrop-blur-sm shadow-sm flex flex-col justify-between">
              <div>
                <h3 className="font-bold text-foreground group-hover:text-primary transition-colors text-lg mb-3 pb-1.5 border-b border-border/10">
                  {getTranslation(contentData.reqEligibleTitle)}
                </h3>
                <p className="text-xs md:text-sm text-muted-foreground leading-relaxed">
                  {getTranslation(contentData.reqEligibleDesc)}
                </p>
              </div>
            </div>
            ...
            <div className="p-6 rounded-2xl border border-border/50 dark:border-white/15 bg-background/45 backdrop-blur-sm shadow-sm flex flex-col justify-between">
              <div>
                <h3 className="font-bold text-foreground text-lg mb-3 pb-1.5 border-b border-border/10 text-destructive">
                  {getTranslation(contentData.reqNotEligibleTitle)}
                </h3>
                <p className="text-xs md:text-sm text-muted-foreground leading-relaxed">
                  {getTranslation(contentData.reqNotEligibleDesc)}
                </p>
              </div>
            </div>
            ...
            <div className="p-6 rounded-2xl border border-border/50 dark:border-white/15 bg-background/45 backdrop-blur-sm shadow-sm flex flex-col justify-between">
              <div>
                <h3 className="font-bold text-foreground group-hover:text-primary transition-colors text-lg mb-3 pb-1.5 border-b border-border/10">
                  {getTranslation(contentData.reqNeedTitle)}
                </h3>
                <p className="text-xs md:text-sm text-muted-foreground leading-relaxed">
                  {getTranslation(contentData.reqNeedDesc)}
                </p>
              </div>
            </div>
          </div>
