...
export async function generateMetadata(props: PageProps): Promise<Metadata> {
  ...
  return {
    ...
    alternates: {
      canonical: `https://www.mcsc.co.id/${lang}/services/business-license`,
      languages: {
        "en": `https://www.mcsc.co.id/en/services/business-license`,
        "id": `https://www.mcsc.co.id/id/services/business-license`,
        "zh": `https://www.mcsc.co.id/cn/services/business-license`,
...
export default async function BusinessLicensePage(props: PageProps) {
  const params = await props.params;
      ...
      <div className="container mx-auto px-4 pt-8 pb-0 max-w-6xl text-left">
        ...
        <section id="licenses-needed" className="scroll-mt-24 mb-16">
          <h2 className="text-3xl font-bold mb-6 text-foreground border-b border-border/50 dark:border-white/10 pb-3">
            {getTranslation(contentData.licensesTitle)}
          </h2>
          <p className="text-base text-muted-foreground leading-relaxed mb-6">
            {getTranslation(contentData.licensesDesc)}
          </p>
        ...
        <section id="nib-details" className="scroll-mt-24 mb-16">
          <div className="p-8 rounded-2xl border border-border/50 dark:border-white/15 bg-background/40 hover:border-primary/40 hover:bg-primary/5 transition-all duration-300 backdrop-blur-sm shadow-sm group">
            <div className="flex items-center gap-3 mb-4">
              ...
              <h2 className="text-2xl font-bold text-foreground group-hover:text-primary transition-colors">
                {getTranslation(contentData.nibTitle)}
              </h2>
            </div>
            
            <p className="text-base text-muted-foreground leading-relaxed mb-6">
              {getTranslation(contentData.nibDesc)}
            </p>

            <h3 className="font-bold text-foreground mb-3 text-sm uppercase tracking-wider">
              {getTranslation(contentData.handlesTitle)}
            </h3>
            <ul className="space-y-3">
              {contentData.nibBullets.map((bullet, idx) => (
        ...
        <section id="npwp-details" className="scroll-mt-24 mb-16">
          <div className="p-8 rounded-2xl border border-border/50 dark:border-white/15 bg-background/40 hover:border-primary/40 hover:bg-primary/5 transition-all duration-300 backdrop-blur-sm shadow-sm group">
            <div className="flex items-center gap-3 mb-4">
              ...
              <h2 className="text-2xl font-bold text-foreground group-hover:text-primary transition-colors">
                {getTranslation(contentData.npwpTitle)}
              </h2>
            </div>
            
            <p className="text-base text-muted-foreground leading-relaxed mb-6">
              {getTranslation(contentData.npwpDesc)}
            </p>
            ...
            <ul className="space-y-3">
              {contentData.npwpBullets.map((bullet, idx) => (
                <li key={idx} className="flex items-start gap-2.5 text-sm text-muted-foreground">
                  <span className="h-1.5 w-1.5 rounded-full bg-primary mt-2 shrink-0" />
                  <span>{getTranslation(bullet)}</span>
                </li>
        ...
        <section id="pkp-details" className="scroll-mt-24 mb-16">
          <div className="p-8 rounded-2xl border border-border/50 dark:border-white/15 bg-background/40 hover:border-primary/40 hover:bg-primary/5 transition-all duration-300 backdrop-blur-sm shadow-sm group">
            <div className="flex items-center gap-3 mb-4">
              ...
              <h2 className="text-2xl font-bold text-foreground group-hover:text-primary transition-colors">
                {getTranslation(contentData.pkpTitle)}
              </h2>
            </div>
            
            <p className="text-base text-muted-foreground leading-relaxed mb-6">
              {getTranslation(contentData.pkpDesc)}
            </p>
            ...
            <ul className="space-y-3 mb-6">
              {contentData.pkpBullets.map((bullet, idx) => (
                <li key={idx} className="flex items-start gap-2.5 text-sm text-muted-foreground">
                  <span className="h-1.5 w-1.5 rounded-full bg-primary mt-2 shrink-0" />
                  <span>{getTranslation(bullet)}</span>
                </li>
