...
import {
  Search, Briefcase, Package, ChevronLeft, ChevronRight,
  ChevronsLeft, ChevronsRight, Loader2, X, ChevronDown,
  Globe, RotateCcw
} from "lucide-react"
...
export default function BrandClassificationPage() {
  ...
  const [classesData, setClassesData] = useState<Record<string, NiceClass> | null>(null)
  ...
  const localUI = {
    en: {
      ...
      pageDesc: "Identify and classify for your trademark application. Search across all classes to find the perfect matches.",
      ...
      searchTip: "Try typing a different keyword (e.g., 'kopi', 'software', 'hukum', 'delivery').",
      ...
      clearAll: "Clear All",
      emptyCart: "No items selected yet. Click the '+' button next to items to add them to your trademark application checklist.",
    ...
    id: {
      ...
      pageDesc: "Identifikasi dan klasifikasikan kelas Nice yang tepat untuk permohonan merek Anda. Cari di semua kelas untuk menemukan kecocokan yang sempurna.",
      searchPlaceholder: "Cari barang atau jasa dalam Bahasa Indonesia atau Inggris...",
      ...
      searchTip: "Coba ketik kata kunci lain (misalnya, 'kopi', 'software', 'hukum', 'pengiriman').",
      ...
      emptyCart: "Belum ada item terpilih. Klik tombol '+' di samping item untuk menambahkan ke daftar permohonan merek Anda.",
    ...
    cn: {
      pageTitle: "商标分类",
      pageDesc: "识别并归类您的商标申请所需的尼斯分类。在所有 45 个类别中进行搜索以找到最匹配的类别。",
      ...
      searchTip: "尝试输入其他关键词（例如：'kopi'、'software'、'hukum'、'delivery'）。",
      ...
      copyList: "复制已选列表",
      clearAll: "全部清除",
      emptyCart: "尚未选择任何项目。点击项目旁的 '+' 按钮，将其添加到您的商标申请清单中。",
  ...
  const classList = useMemo(() => {
    ...
    const sortedKeys = Object.keys(classesData).sort((a, b) => parseInt(a) - parseInt(b))
  ...
  const filteredItems = useMemo(() => {
    ...
    return filtered.sort((a, b) => {
      ...
      const aWordBoundary = wordBoundaryRegex.test(aId) || wordBoundaryRegex.test(aEn)
      const bWordBoundary = wordBoundaryRegex.test(bId) || wordBoundaryRegex.test(bEn)
      ...
      <section className="pb-24 flex-grow container mx-auto px-4">
        {isLoading ? (
          ...
          <div className="grid grid-cols-1 md:grid-cols-4 gap-8 max-w-7xl mx-auto h-[600px]">
            <Card className="col-span-1 border-none bg-white/40 dark:bg-background/50 backdrop-blur-md p-6 h-full hidden md:block border border-zinc-200/50 dark:border-zinc-800/50">
              <div className="animate-pulse space-y-4">
                <div className="h-6 bg-zinc-200 dark:bg-zinc-800 rounded w-2/3"></div>
                <div className="space-y-2 pt-4">
                  {[...Array(12)].map((_, i) => (
                    <div key={i} className="h-10 bg-zinc-100 dark:bg-zinc-900 rounded-lg"></div>
                  ))}
                </div>
              </div>
            </Card>
            <Card className="col-span-1 md:col-span-3 border-none bg-white/40 dark:bg-background/50 backdrop-blur-md p-8 h-full flex flex-col justify-center items-center border border-zinc-200/50 dark:border-zinc-800/50">
              <Loader2 className="h-12 w-12 text-primary animate-spin mb-4" />
              <p className="text-zinc-500 dark:text-muted-foreground font-medium text-lg animate-pulse">{ui.loadingText}</p>
            </Card>
          </div>
        ) : error || !classesData ? (
          /* Error State */
          <div className="max-w-md mx-auto text-center py-20">
            <RotateCcw className="h-12 w-12 text-destructive mx-auto mb-4 cursor-pointer hover:rotate-180 transition-all duration-300" onClick={() => window.location.reload()} />
          ...
          <div className="grid grid-cols-1 lg:grid-cols-4 gap-8 max-w-7xl mx-auto items-start">
                    ...
                    {classList.goods.map(cls => (
                      ...
                      >
                        <span className="truncate pr-2">
                          {ui.classTab.replace("{id}", cls.id.padStart(2, "0"))} - {cls.desc}
                        </span>
                        <Package className={`h-4.5 w-4.5 shrink-0 opacity-40 group-hover:opacity-100 transition-opacity ${activeClass === cls.id && !globalSearch ? "text-primary opacity-100" : ""}`} />
                      </button>
                    ))}
                  </div>
                </div>
                  ...
                  <div className="space-y-1">
                    {classList.services.map(cls => (
                      ...
                      >
                        ...
                        <Briefcase className={`h-4.5 w-4.5 shrink-0 opacity-40 group-hover:opacity-100 transition-opacity ${activeClass === cls.id && !globalSearch ? "text-primary opacity-100" : ""}`} />
                      </button>
                    ))}
                  </div>
                </div>
            ...
            <div className="col-span-1 lg:col-span-3 space-y-6">
              ...
              {!globalSearch && classesData?.[activeClass] && (
                ...
                >
                  <BorderGlow
                    ...
                    glowIntensity={0.8}
                  >
                    <Card className="border-none bg-white/50 dark:bg-background/40 border border-zinc-200/50 dark:border-zinc-800/40 backdrop-blur-md p-6 relative overflow-hidden w-full shadow-sm dark:shadow-none">
                      ...
                      <div className="flex items-start gap-4">
                        ...
                        <div>
                          <span className="text-xs uppercase font-extrabold tracking-widest text-primary">
                            {parseInt(activeClass) <= 34 ? ui.goodsSection.split(" (")[0] : ui.servicesSection.split(" (")[0]}
                          </span>
                          <h2 className="text-2xl font-bold font-sans mt-0.5 mb-2">
                            {ui.classTab.replace("{id}", activeClass)}
                          </h2>
                          <h4 className="text-sm font-semibold text-primary/80 mb-1">{ui.classDescHeader}</h4>
                    ...
                    <div className="flex items-center gap-4">
                      ...
                      <div>
                        ...
                        <p className="text-xs text-muted-foreground mt-1">
                          Searching across all 45 Nice classes simultaneously. Results sorted by relevancy.
                        </p>
                      </div>
                    </div>
                  </Card>
              ...
              <BorderGlow
                ...
                glowIntensity={0.8}
              >
                <Card className="border-none bg-white/50 dark:bg-background/40 border border-zinc-200/50 dark:border-zinc-800/40 backdrop-blur-md overflow-hidden w-full shadow-sm dark:shadow-none">
                  <div className="p-4 border-b border-zinc-200 dark:border-zinc-800/50 flex items-center justify-between">
                    <span className="text-sm font-bold text-zinc-500 dark:text-muted-foreground">
                      {ui.totalResults.replace("{count}", filteredItems.length.toLocaleString())}
                    </span>
                  </div>
                  ...
                  {filteredItems.length === 0 ? (
                    ...
                    <div className="text-center py-16 px-4">
                      ...
                      <p className="text-lg font-bold text-zinc-900 dark:text-foreground mb-1">{ui.noResults}</p>
                      ...
                      <table className="w-full text-left border-collapse text-sm">
                        <thead>
                          <tr className="border-b border-zinc-200 dark:border-zinc-800/80 bg-zinc-100/80 dark:bg-zinc-950/40 text-zinc-700 dark:text-muted-foreground font-bold">
                            {globalSearch && (
                              <th className="py-4 px-4 font-bold uppercase tracking-wider text-xs w-16">{ui.tableColClass}</th>
                            )}
                            <th className="py-4 px-4 font-bold uppercase tracking-wider text-xs w-[50%]">{ui.tableColNameId}</th>
                            <th className="py-4 px-4 font-bold uppercase tracking-wider text-xs w-[50%]">{ui.tableColNameEn}</th>
                          </tr>
                        </thead>
                        <tbody>
                              ...
                              >
                                {globalSearch && (
                                  <td className="py-4 px-4 font-bold shrink-0">
                                    <span className="inline-flex h-8 w-8 items-center justify-center rounded-lg bg-zinc-200 dark:bg-zinc-800/80 text-zinc-900 dark:text-foreground border border-zinc-200 dark:border-zinc-700/50 text-xs shadow-sm font-sans" title={classesData?.[itemClass]?.description || ""}>
                                      {itemClass}
                                    </span>
                                  </td>
                                )}
                  ...
                  {totalPages > 1 && (
                    <div className="p-4 border-t border-zinc-200 dark:border-zinc-900/60 flex flex-col md:flex-row gap-4 items-center justify-between bg-zinc-50/40 dark:bg-zinc-950/20 backdrop-blur-xs">
                      <span className="text-xs text-zinc-600 dark:text-muted-foreground font-semibold uppercase tracking-wider">
                        Page {currentPage} of {totalPages} (Showing {itemsPerPage * (currentPage - 1) + 1} - {Math.min(itemsPerPage * currentPage, filteredItems.length)} of {filteredItems.length})
                      </span>
