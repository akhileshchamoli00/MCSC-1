...
import {
  ...
  User, Briefcase, Building, Calendar, Sunrise, Sun, Moon, Cake
} from "lucide-react";
              ...
              <div className="space-y-4 max-w-2xl">
                ...
                <div className="space-y-2">
                  <h1 className={`text-3xl sm:text-5xl font-black tracking-tight leading-none bg-clip-text text-transparent bg-gradient-to-r pb-1 ${isBday ? bdayTextGradient : "from-zinc-700 via-slate-700 to-zinc-900 dark:from-sky-300 dark:via-blue-200 dark:to-indigo-300"}`}>
                    {isBday ? `Happy Birthday, ${birthdayData.employee_name}! 🎂` : `${greetingData.greeting}, ${profile ? `${profile.first_name} ${profile.last_name}` : "Team Member"}`}
                  </h1>
                    ...
                    {isBday ? (
                      <>
                        ...
                        <span className="text-xs text-muted-foreground/80 mt-1 block">
                          You have <span className="font-bold text-zinc-950 dark:text-sky-300 underline decoration-zinc-500/30 dark:decoration-sky-400/30 underline-offset-4">{data.pending_leaves || 0} pending leave requests</span> and your attendance is at <span className="font-bold text-zinc-950 dark:text-sky-300 underline decoration-zinc-500/30 dark:decoration-sky-400/30 underline-offset-4">{data.attendance_pct || 0}%</span> this month.
                        </span>
                      </>
                    ) : (
                      <>
