...
export async function POST(req: Request) {
  ...
  if (smtpUser && smtpPassword && smtpHost) {
    try {
      ...
      const mailOptions = {
        from: `"${senderName} Status Check" <${senderEmail}>`,
        to: "admin@mcsc.co.id",
      ...
      return NextResponse.json({ success: true, message: "Status query submitted successfully!" })
    } catch (directErr: any) {
  ...
  try {
    ...
    const backendRes = await fetch(`${backendUrl}/api/status-check`, {
      method: "POST",
    ...
    if (backendRes.ok && backendData.success) {
      return NextResponse.json({ success: true, message: "Status query submitted successfully via backend dispatcher!" })
    }

    return NextResponse.json(
      { error: backendData.detail || backendData.message || backendData.error || "Failed to send query via backend dispatcher" },
